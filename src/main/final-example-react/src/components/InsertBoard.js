...
const InsertBoard = () => {
    ...
    const insertBoard = useCallback((e) => {
        ...
        const insertBoardAxios = async () => {
            try {
                const response = await axios.post('http://localhost:8081/vod/board', formDataObj, {
                    headers: {
    ...
    <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
        ...
        <form id="insertForm" onSubmit={insertBoard}>
            ...
            <tr>
                ...
                <td style={{textAlign: "left"}}>
                <input type="text" name="writer" readonly value={sessionStorage.getItem("userName")}></input>
                </td>
            </tr>
            <tr>
                <td style={{background: 'skyblue', width: '70px'}}>
