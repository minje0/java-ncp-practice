...
const Board = () => {
    ...
    useEffect(() => {
        const getBoard = async () => {
            try {
                const response = await axios.get(`http://localhost:8081/vod/board/${boardNo}`, {
                    headers: {
    ...
    useEffect(() => {
        boardFileList && boardFileList.forEach(boardFile => {
            const originBoardFile = {
                vodBoardNo: board.boardNo,
                vodfileNo: boardFile.boardFileNo,
                vodOriginName: boardFile.vodOriginName,
            }
    ...
    const deleteBoard = useCallback((e) => {
        ...
        const deleteBoardAxios = async () => {
            try {
                const response = await axios.delete(`http://localhost:8081/board/board/${boardNo}`, {
                    headers: {
                        Authorization: `Bearer ${sessionStorage.getItem("ACCESS_TOKEN")}`
                    }
                });
    ...
    const updateBoard = useCallback((e) => {
        ...
        formDataObj.changeFileList = changeFileList;
        formDataObj.originFileList = JSON.stringify(originFileList);

        const updateBoardAxios = async () => {
            try {
                const response = await axios.put('http://localhost:9090/board/board', formDataObj, {
                    headers: {
                        Authorization: `Bearer ${sessionStorage.getItem("ACCESS_TOKEN")}`,
    ...
    }, [board, boardFileList, originFileList, changeFileList]);
  return (
            ...
            <table style={{borderCollapse: 'collapse', border: '1px solid black'}}>
                <tr>
                    ...
                    <td style={{textAlign: 'left'}}>
                        <input type="text" name="boardTitle" id="boardTitle" value={board ? board.title : ''}></input>
                    </td>
                </tr>
                <tr>
                    ...
                    <td style={{textAlign: 'left'}}>
                        <input type="text" name="boardWriter" id="boardWriter" readonly value={board ? board.writer : ''}></input>
                    </td>
                </tr>
                <tr>
                    <td style={{background: 'skyblue'}}>
                    ...
                    <td style={{textAlign: 'left'}}>
                        <textarea name="boardContent" id="boardContent" cols="40" rows="10" value={board ? board.content : ''}></textarea>
                    </td>
                </tr>
                <tr>
                    <td style={{background: 'skyblue'}}>
